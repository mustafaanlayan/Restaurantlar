...
namespace Restaurantt.DataAcces.Dals.Base
{
    public class Repository<TEntity> : IRepository<TEntity> where TEntity : class, IEntity, new()
    {
        ...
        {
            return filter == null ? _context.Set<TEntity>().MultipleInclude(includes).Select(selector) : _context.Set<TEntity>().Where(filter).Select(selector);
        }
        ...
        {
            return filter==null? _context.Set<TEntity>().Select(selector):_context.Set<TEntity>().MultipleInclude(includes).Where(filter).Select(selector);
        }
